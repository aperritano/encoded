...
class File(Item):
    ...
    rev = {
        'input_to_step_run': ('AnalysisStepRun', 'input_files'),
    ...
    @calculated_property(schema={
        "title": "Step runs using this file",
        "description": "The step run(s) which use this file as input.",
        "comment": "Do not submit. Values in the list are reverse links from step runs.",
        "type": "array",
        "items": {
            "type": ['string', 'object'],
            "linkFrom": "AnalysisStepRun.input_files",
        },
        "notSubmittable": True,
    })
    def input_to_step_run(self, request, input_to_step_run):
        return paths_filtered_by_status(request, input_to_step_run)
