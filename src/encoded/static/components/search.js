...
const Term = (props) => {
    const { filters, facet, total, searchBase, onFilter, statusFacet } = props;
                                    ...
                                    <div className="facet__terms">
                                        ...
                                        <div className={`term-list search${titleComponent.replace(/\s+/g, '')}`} onScroll={shadeOverflowOnScroll}>
                                            {filteredList.map(term =>
                                                <TermComponent {...this.props} key={term.key} term={term} filters={filters} total={total} statusFacet={statusFacet} />
                                            )}
                                        </div>
...
export const FacetList = (props, reactContext) => {
    ...
    // Filter out the "type" facet if the query string has exactly one "type=" in it.
    const renderableFacets = query.getKeyValues('type').length === 1 ? facets.filter(facet => facet.field !== 'type') : facets;
        ...
        <div className="search-results__facets">
            <div className={`box facets${addClasses ? ` ${addClasses}` : ''}`}>
                <div className={`orientation${orientation === 'horizontal' ? ' horizontal' : ''}`}>
                    ...
                    {renderableFacets.map((facet) => {
                        const FacetRenderer = FacetRegistry.Facet.lookup(facet.field);
                        return FacetRenderer && <FacetRenderer key={facet.field} facet={facet} results={context} pathname={parsedUrl.pathname} queryString={parsedUrl.query} />;
                    })}
                </div>
            </div>
        </div>
