 ...
 * The initial question mark does not get included, nor does it get accepted when creating the
 * QueryString object.
 *
 * Internally, values of key/value pairs get stored in their decoded forms (i.e. spaces are
 * spaces). They get decoded when a QueryString object gets created, and only get encoded in
 * format().
 */
...
class QueryString {
     ...
     * where the first element is the key and the second is the value. For example, the query
     * string "type=Experiment&organism=human" becomes:
     ...
     * @param {*} value Non-encoded value to add to query string. Numbers converted to strings.
     *
     * @return {object} Reference to this object for method chaining.
     */
    ...
    /**
     * Delete any key/value pairs matching the given `key` and `value`, or just all matching keys
     * regardless of their value if you don't pass `value`.
     * @param {string} key Key value to delete
     * @param {string} value Optional non-encoded value to also match to delete
     *
     * @return {object} Reference to this object for method chaining.
     */
     ...
     * key/value pairs match key, they all get removed and replaced by this new one.
     * @param {string} key Key value to replace
     * @param {string} value Non-encoded value to replace
     *
     ...
     *
     * @return {array} Non-encoded values that have `key` as their key.
     */
    getKeyValues(key) {
